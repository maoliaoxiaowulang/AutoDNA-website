    ...
    <div id="vw-hud" ref="hudRef" class="hidden">
      ...
      <div class="hud-bottom">
        <div id="vw-playControls" ref="playControlsRef">
          ...
          <input type="range" ref="frameSliderRef" min="0" max="0" value="0" @input="gotoFrameInput">
        ...
        <input type="text" ref="jumpInputRef" class="jump-input" placeholder="第几帧" @keydown.enter="doJump">
...
// ── Palette ───────────────────────────────────────────────────────────────────
const PAL = [
  0x1565c0, 0x2e7d32, 0xc77800, 0x6a1b9a, 0xb71c1c, 0x006064,
  0xe65100, 0x1a237e, 0x33691e, 0x880e4f, 0x004d40, 0x4a148c,
  0x827717, 0x3e2723, 0x01579b, 0x37474f,
]

// ── State ─────────────────────────────────────────────────────────────────────
let pendTop = null, pendDat = null, pendDatFile = null
const LARGE_FILE = 30 * 1024 * 1024
let particles = null, frames = null, frameIdx = 0
let playing = false, playTimer = null
let trajId = null, totalFrames = 0, frameCache = {}, loadedChunks = new Set()
...
// geometry cache
let SPHERE_BB = null, SPHERE_BASE = null, CYL = null
const YAXIS_G = { x: 0, y: 1, z: 0 }

// ── Helpers ───────────────────────────────────────────────────────────────────
function getW () { return wrapRef.value?.clientWidth  ?? window.innerWidth  }
function getH () { return wrapRef.value?.clientHeight ?? window.innerHeight }
...
function onDragLeave (e) { if (!wrapRef.value?.contains(e.relatedTarget)) dragOver.value = false }
...
async function uploadAndInit (datFile, topText) {
  ...
  function setProg (t, pct) {
    txt.textContent = t
    if (pct != null) { prog.style.width = pct + '%'; progTrackRef.value.style.display = 'block' }
    else { progTrackRef.value.style.display = 'none' }
  }
  ...
  const data = await new Promise((resolve, reject) => {
    ...
    xhr.upload.onload = () => {
      let pct = 52
      setProg('建立帧索引…', pct)
      _animTimer = setInterval(() => { pct = Math.min(pct + 0.25, 88); prog.style.width = pct + '%' }, 300)
    }
    xhr.onload = () => { clearInterval(_animTimer); xhr.status === 200 ? resolve(JSON.parse(xhr.responseText)) : reject(new Error('HTTP ' + xhr.status)) }
    xhr.onerror = () => { clearInterval(_animTimer); reject(new Error('网络错误')) }
    xhr.open('POST', '/api/traj/upload')
    xhr.send(fd)
  })

  if (data.error) { ld.classList.remove('show'); progTrackRef.value.style.display = 'none'; alert('错误：' + data.error); return }

  setProg('加载首批帧…', 92)
  trajId = data.id; totalFrames = data.n_frames
  particles = data.particles; frameCache = {}; loadedChunks = new Set()
  frames = null
  await fetchChunk(0)
  ...
  ld.classList.remove('show'); progTrackRef.value.style.display = 'none'
}
...
async function fetchChunk (chunkIdx) {
  ...
  try {
    ...
    if (data.error) { console.error(data.error); loadedChunks.delete(chunkIdx); return }
    const N = particles.length
    data.frames.forEach((raw, i) => { frameCache[start + i] = frameFromBackend(raw, N) })
  ...
  } catch (e) { console.error(e); loadedChunks.delete(chunkIdx) }
}
...
function parseTop (text) {
  ...
  for (let i = 1; i < lines.length && parts.length < N; i++) {
    ...
    parts.push({ id: parts.length, strand: parseInt(v[0]) - 1, base: v[1], n3: parseInt(v[2]), n5: parseInt(v[3]) })
  }
  return parts
}
...
function parseConf (text) {
  ...
  while (i < lines.length) {
    ...
    if (!l.startsWith('t')) { i++; continue }
    const t = parseFloat(l.split('=')[1])
    const bv = (lines[i + 1] || '').trim().split(/\s+/)
    const box = { x: parseFloat(bv[2]) || 1e9, y: parseFloat(bv[3]) || 1e9, z: parseFloat(bv[4]) || 1e9 }
    i += 3
    const pos = [], a1 = [], a3 = []
    while (i < lines.length) {
...
function initThree () {
  ...
  controls.minDistance = 0.5; controls.maxDistance = 800

  scene.add(new THREE.AmbientLight(0xffffff, 0.70))
  const kl = new THREE.DirectionalLight(0xffffff, 1.2); kl.position.set(10, 20, 15); scene.add(kl)
  const fl = new THREE.DirectionalLight(0xffffff, 0.3); fl.position.set(-10, -8, -12); scene.add(fl)

  _axisCvs = axisCanvasRef.value
  _axisCvs.width = 140; _axisCvs.height = 140

  let _mouseDownPos = { x: 0, y: 0 }
  renderer.domElement.addEventListener('mousedown', e => {
    if (e.button === 0) { _mouseDownPos.x = e.clientX; _mouseDownPos.y = e.clientY }
  })
...
function renderAxisOverlay () {
  ...
  const W = 140, H = 140, cx = W / 2, cy = H / 2, arm = 38
  ...
  proj.forEach(ax => {
    const dx = ax.ex - cx, dy = ax.ey - cy
    const segLen = Math.sqrt(dx * dx + dy * dy)
    if (segLen < 1e-4) return
    ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(ax.ex, ax.ey)
    ctx.strokeStyle = ax.col; ctx.lineWidth = 1.6; ctx.stroke()
    const nx = dx / segLen, ny = dy / segLen, hlen = 9, hwidth = 3.5
    const px = -ny * hwidth, py = nx * hwidth
    ctx.beginPath(); ctx.moveTo(ax.ex, ax.ey)
    ctx.lineTo(ax.ex - nx * hlen + px, ax.ey - ny * hlen + py)
    ctx.lineTo(ax.ex - nx * hlen - px, ax.ey - ny * hlen - py)
    ctx.closePath(); ctx.fillStyle = ax.col; ctx.fill()
    const lx = cx + dx * 1.38, ly = cy + dy * 1.38
    ctx.fillStyle = ax.col; ctx.font = '600 10px "Segoe UI",system-ui,sans-serif'
...
function handleCanvasClick (e) {
  ...
  for (let mi = 0; mi < allBBMeshes.length; mi++) {
    ...
    let ok = true, _o = entry.mesh
    while (_o) { if (_o.visible === false) { ok = false; break }; _o = _o.parent }
    if (!ok) continue
    const hits = ray.intersectObject(entry.mesh, false)
    if (hits.length > 0 && hits[0].distance < bestDist) { bestDist = hits[0].distance; best = { entry, instanceId: hits[0].instanceId } }
  }
  if (!best) return
  ...
  if (selectedNuc && selectedNuc.globalIdx === gIdx) {
    ...
    selectedNuc = null; setTracking(false)
    selInfoRef.value.style.display = 'none'
  } else {
    if (selectedNuc) restoreNucColor(selectedNuc)
    selectedNuc = { strandIdx: best.entry.strand, localIdx: best.instanceId, globalIdx: gIdx, entry: best.entry }
    applyNucColor(best.entry, best.instanceId, getSelColor())
    const base = particles ? particles[gIdx].base : '?'
    selInfoRef.value.textContent = '选中：核苷酸 ' + (gIdx + 1) + ' (' + base + ')  链 ' + (best.entry.strand + 1)
...
function applyNucColor (entry, localIdx, col) {
  ...
  entry.mesh.setColorAt(localIdx, sphere); entry.mesh.instanceColor.needsUpdate = true
  if (entry.stickMesh && entry.stickMesh.instanceColor) {
    const stick = col ? col : new THREE.Color(PAL[entry.strand % PAL.length]).multiplyScalar(0.80)
    entry.stickMesh.setColorAt(localIdx, stick); entry.stickMesh.instanceColor.needsUpdate = true
  }
}
...
function setTracking (on) {
  ...
  if (!on) return
  if (selectedNuc === null) { tracking = false; trackBtnRef.value?.classList.remove('active'); return }
  const gIdx = selectedNuc.globalIdx
  const nucPos = nucWorldPos[gIdx], a1v = nucWorldA1[gIdx]
  if (!nucPos || !a1v) { tracking = false; return }
  const right = a1v.clone().normalize()
  let tempUp = new THREE.Vector3(0, 1, 0)
  if (Math.abs(right.dot(tempUp)) > 0.9) tempUp.set(1, 0, 0)
  const fwd = new THREE.Vector3().crossVectors(right, tempUp).normalize()
  ...
  camera.up.copy(up); controls.update()
}

function toggleTracking () {
  if (!selectedNuc) { alert('请先点击选择一个核苷酸'); return }
...
function patchPerInstanceAlpha (mat, n) {
  ...
  for (let i = 0; i < n; i++) { alphaData[i*4] = 255; alphaData[i*4+3] = 255 }
...
function getBaseColor (b) { return BASE_COL[b.toUpperCase()] || 0x78909c }

function getSphereBB   () { if (!SPHERE_BB)   SPHERE_BB   = new THREE.SphereGeometry(0.20, 12, 8); return SPHERE_BB }
function getSphereBase () { if (!SPHERE_BASE) SPHERE_BASE = new THREE.SphereGeometry(0.1,  10, 7); return SPHERE_BASE }
function getCyl        () { if (!CYL)         CYL         = new THREE.CylinderGeometry(1, 1, 1, 8, 1); return CYL }
...
function buildCylMesh (segs, radius, color, withAlpha) {
  ...
  if (n === 0) return null
  const mat = new THREE.MeshPhysicalMaterial({ color, roughness: 0.50, metalness: 0.0, clearcoat: 0.35, clearcoatRoughness: 0.18 })
  let alphaInfo = null
  ...
  for (let i = 0; i < n; i++) {
    const o = i * 6
    pA.set(segs[o], segs[o+1], segs[o+2]); pB.set(segs[o+3], segs[o+4], segs[o+5])
    setCylPose(dummy, pA, pB, radius); mesh.setMatrixAt(i, dummy.matrix)
  }
...
function clearObjs () {
  if (objs.main) {
    ...
    objs.main.traverse(child => {
      ...
      if (child.geometry && child.geometry !== SPHERE_BB && child.geometry !== SPHERE_BASE && child.geometry !== CYL)
...
function unwrapPositions (pos, parts, box) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    while (true) {
      ...
      dx -= bx * Math.round(dx / bx); dy -= by * Math.round(dy / by); dz -= bz * Math.round(dz / bz)
...
function buildScene (parts, frame) {
  ...
  const BS_LEN = 0.48
  const bs = []
  for (let i = 0; i < N; i++) bs.push(new THREE.Vector3().copy(pos[i]).addScaledVector(a1[i], BS_LEN))

  nucWorldPos = []; nucWorldA1 = []
  for (let i = 0; i < N; i++) { nucWorldPos[i] = pos[i].clone(); nucWorldA1[i] = a1[i].clone() }
  ...
  const mainGroup = new THREE.Group(), strandGroups = {}

  Object.keys(strandNucs).forEach(sKey => {
    const s = parseInt(sKey), nucs = strandNucs[s]
    const sc = new THREE.Color(PAL[s % PAL.length])
    const sGroup = new THREE.Group()

    const bbMat = new THREE.MeshPhysicalMaterial({ roughness: 0.55, metalness: 0.0, clearcoat: 0.45, clearcoatRoughness: 0.12 })
    ...
    for (let k = 0; k < nucs.length; k++) {
      const idx = nucs[k]
      dummy.position.copy(pos[idx]); dummy.scale.setScalar(1); dummy.updateMatrix()
      bbSph.setMatrixAt(k, dummy.matrix)
      bbSph.setColorAt(k, (selectedNuc && selectedNuc.globalIdx === idx) ? getSelColor() : sc)
    }
    bbSph.instanceMatrix.needsUpdate = true; bbSph.instanceColor.needsUpdate = true
    const nucSub = new THREE.Group(); nucSub._isNuc = true
    nucSub.add(bbSph); nucSub.visible = layers.nuc; sGroup.add(nucSub)

    const baseMat = new THREE.MeshPhysicalMaterial({ roughness: 0.45, metalness: 0.0, clearcoat: 0.35, clearcoatRoughness: 0.15 })
    ...
    for (let k = 0; k < nucs.length; k++) {
      const idx = nucs[k]
      dummy.position.copy(bs[idx]); dummy.scale.setScalar(1); dummy.updateMatrix()
      baseSph.setMatrixAt(k, dummy.matrix)
      col.setHex(getBaseColor(parts[idx].base)); baseSph.setColorAt(k, col)
    }
    baseSph.instanceMatrix.needsUpdate = true; baseSph.instanceColor.needsUpdate = true
    const baseSub = new THREE.Group(); baseSub._isBase = true
    baseSub.add(baseSph); baseSub.visible = layers.base; sGroup.add(baseSub)

    const bbSegs = [], stSegs = []
    for (let k = 0; k < nucs.length; k++) {
      const idx = nucs[k], p = parts[idx]
      stSegs.push(pos[idx].x, pos[idx].y, pos[idx].z, bs[idx].x, bs[idx].y, bs[idx].z)
      if (p.n3 >= 0 && p.n3 < N) bbSegs.push(pos[idx].x, pos[idx].y, pos[idx].z, pos[p.n3].x, pos[p.n3].y, pos[p.n3].z)
    }
    const c = new THREE.Color(PAL[s % PAL.length]).multiplyScalar(0.80)
    const bbSub = new THREE.Group(); bbSub._isBB = true
    const m1 = buildCylMesh(bbSegs, 0.10, c, false)
    ...
    if (m2) {
      ...
      m2.instanceColor.needsUpdate = true; bbSub.add(m2)
    }
    bbSub.visible = layers.bb; sGroup.add(bbSub)

    const entry = { mesh: bbSph, baseMesh: baseSph, stickMesh: m2 || null, nucs, strand: s }
    allBBMeshes.push(entry)
    if (selectedNuc) {
      for (let k = 0; k < nucs.length; k++) {
        if (nucs[k] === selectedNuc.globalIdx) { selectedNuc.entry = entry; selectedNuc.localIdx = k; break }
      }
    }
    sGroup.visible = (strandVisible[s] !== false)
    strandGroups[s] = sGroup; mainGroup.add(sGroup)
  })

  scene.add(mainGroup)
  objs.main = mainGroup; objs.strands = strandGroups
}
...
function gotoFrameInput (e) { gotoFrame(+e.target.value) }
...
function doJump () {
  ...
  if (jumpMode === 'frame') {
    ...
    src.forEach((f, i) => { const d = Math.abs(f.t - n); if (d < bd) { bd = d; best = trajId ? keys[i] : i } })
    gotoFrame(best)
  }
}
...
function togglePlay () {
  ...
  if (playing) {
    const N = trajId ? totalFrames : frames.length
    playTimer = setInterval(() => { gotoFrame((frameIdx + 1) % N) }, 80)
...
function toggleLayer (name) {
  ...
  if (objs.main) {
    if (name === 'bb')   objs.main.traverse(o => { if (o._isBB)   o.visible = layers.bb })
    if (name === 'nuc')  objs.main.traverse(o => { if (o._isNuc)  o.visible = layers.nuc })
    if (name === 'base') objs.main.traverse(o => { if (o._isBase) o.visible = layers.base })
  }
...
function buildStrandToggles (parts) {
  const bar = strandBarRef.value; bar.innerHTML = ''
  const seen = {}, strands = []
  parts.forEach(p => { if (!seen[p.strand]) { seen[p.strand] = true; strands.push(p.strand) } })
  strands.sort((a, b) => a - b)
  strands.forEach(s => {
    const hex = '#' + PAL[s % PAL.length].toString(16).padStart(6, '0')
    const btn = document.createElement('button')
    btn.className = 'tog on'
    btn.innerHTML = '<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:' + hex + ';margin-right:5px;vertical-align:middle;flex-shrink:0"></span>链 ' + (s + 1)
    ...
    btn.onclick = () => {
      ...
      grp.visible = !grp.visible; strandVisible[s] = grp.visible
...
// ── Init viewer ───────────────────────────────────────────────────────────────
function initViewer () {
  const ld = loadingRef.value; ld.classList.add('show')
  ...
  setTimeout(() => {
    ...
    try {
      particles = parseTop(pendTop); frames = parseConf(pendDat)
...
function reloadViewer () {
  pendTop = pendDat = pendDatFile = null; particles = frames = null
  trajId = null; totalFrames = 0; frameCache = {}; loadedChunks = new Set()
  strandVisible = {}; selectedNuc = null; allBBMeshes = []; nucWorldPos = []
  setTracking(false)
  strandBarRef.value.innerHTML = ''
  selInfoRef.value.style.display = 'none'
  if (threeReady) clearObjs()
  if (playing) { clearInterval(playTimer); playing = false }
  badgeTopRef.value.textContent = 'topology 未加载'; badgeTopRef.value.classList.remove('ok')
  badgeDatRef.value.textContent = 'trajectory 未加载'; badgeDatRef.value.classList.remove('ok')
...
onMounted(async () => {
  try {
    ...
    await loadScript('https://cdn.jsdelivr.net/npm/three@0.134.0/examples/js/controls/TrackballControls.js')
  } catch {
    cdnErrorRef.value.style.display = 'block'
  }
})
...
onUnmounted(() => {
  ...
  threeReady = false; _animId = null; _resizeHandler = null
  SPHERE_BB = null; SPHERE_BASE = null; CYL = null
})
</script>
...
#vw-dropzone {
  ...
  background:
    ...
    radial-gradient(ellipse 50% 60% at 75% 60%, rgba(6,182,212,0.06), transparent),
    #f1f5f9;
  transition: opacity 0.5s;
}
#vw-dropzone.hidden { opacity: 0; pointer-events: none; }
...
.dz-title {
  font-size: 30px; font-weight: 800; letter-spacing: -0.6px; margin-bottom: 8px;
  background: linear-gradient(110deg,#3730a3 0%,#4338ca 40%,#0f766e 70%,#1d4ed8 100%);
  background-size: 200% auto;
  -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
  animation: vw-shimmer 4s linear infinite;
}
.dz-sub { font-size: 13px; color: #64748b; margin-bottom: 36px; }
@keyframes vw-shimmer { 0%{background-position:0% center} 100%{background-position:200% center} }

.dz-area {
  position: relative; width: 400px; height: 190px; border-radius: 20px;
  display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 10px;
  cursor: pointer; background: rgba(255,255,255,0.60);
  border: 1.5px solid rgba(99,102,241,0.25);
  transition: border-color 0.25s, transform 0.25s, background 0.25s;
}
.dz-area:hover, .dz-area.over {
  border-color: rgba(67,56,202,0.7); background: rgba(255,255,255,0.85);
  transform: scale(1.02);
}
.dz-icon { font-size: 30px; opacity: 0.5; }
.dz-hint { font-size: 14px; color: #475569; }
.dz-hint em { color: #4338ca; font-style: normal; font-weight: 600; }
.dz-hint2 { font-size: 12px; color: #94a3b8; }

.dz-badges { display: flex; gap: 12px; margin-top: 24px; }
.badge {
  padding: 5px 14px; border-radius: 20px; font-size: 12px; font-weight: 600;
  border: 1px solid rgba(99,102,241,0.2); color: #64748b; background: rgba(255,255,255,0.7);
}
.badge.ok { border-color:#15803d; color:#15803d; background:rgba(21,128,61,0.08); }

/* ── CDN error ──────────────────────────────────────────────────────────────── */
#vw-cdnError {
  display: none; position: absolute; bottom: 20px; left: 50%; transform: translateX(-50%);
  background: rgba(239,68,68,0.08); border: 1px solid rgba(239,68,68,0.3);
  border-radius: 10px; padding: 10px 20px; font-size: 13px; color: #b91c1c;
  z-index: 200; text-align: center; max-width: 500px;
}

/* ── HUD ────────────────────────────────────────────────────────────────────── */
#vw-hud { position: absolute; inset: 0; pointer-events: none; z-index: 50; }
#vw-hud.hidden { display: none; }

.hud-bar {
  position: absolute; top: 16px; left: 50%; transform: translateX(-50%);
  background: rgba(255,255,255,0.82); backdrop-filter: blur(20px);
  border: 1px solid rgba(99,102,241,0.15); border-radius: 30px;
  padding: 6px 16px; display: flex; align-items: center; gap: 12px;
  pointer-events: auto; white-space: nowrap;
  box-shadow: 0 2px 12px rgba(0,0,0,0.08);
}
.hud-stat { font-size: 12px; color: #64748b; }
.hud-stat b { color: #4338ca; font-weight: 600; }
.sep { width: 1px; height: 14px; background: rgba(99,102,241,0.15); flex-shrink: 0; }
.tog {
  padding: 3px 10px; border-radius: 6px;
  border: 1px solid rgba(99,102,241,0.22); background: transparent;
  color: #64748b; font-size: 11px; font-weight: 600; cursor: pointer; font-family: inherit;
  transition: all 0.15s;
}
.tog.on { background: rgba(67,56,202,0.10); border-color: #4338ca; color: #3730a3; }
.tog:hover { border-color: #4338ca; color: #3730a3; }
...
.hud-bottom {
  ...
  pointer-events: auto; min-width: 300px; max-width: 640px;
  box-shadow: 0 2px 12px rgba(0,0,0,0.08);
}
#vw-playControls { display: flex; align-items: center; gap: 10px; flex: 1; min-width: 0; }
.jump-input {
  width: 88px; border: 1px solid rgba(99,102,241,0.22); border-radius: 6px;
  padding: 3px 8px; font-size: 11px; font-family: inherit;
  color: #475569; background: rgba(255,255,255,0.80); outline: none;
  transition: border-color 0.15s;
}
.jump-input:focus { border-color: #4338ca; }
.jump-input::placeholder { color: #94a3b8; }
.jump-mode-group {
  display: flex; border: 1px solid rgba(99,102,241,0.22); border-radius: 6px; overflow: hidden;
}
.jump-mode {
...
.jump-mode.on { background: rgba(67,56,202,0.10); color: #3730a3; }
.jump-mode:hover:not(.on) { color: #475569; }
#vw-playBtn {
  background: none; border: none; cursor: pointer;
  color: #4338ca; font-size: 17px; padding: 0 4px; transition: color 0.15s; flex-shrink: 0;
}
#vw-playBtn:hover { color: #3730a3; }
...
#vw-hud input[type="range"]::-webkit-slider-thumb {
  ...
  background: #4338ca; box-shadow: 0 0 8px rgba(67,56,202,0.35); cursor: pointer;
}
.frame-lbl { font-size: 11px; color: #64748b; white-space: nowrap; }
.frame-lbl b { color: #4338ca; }
...
.corner-btn:hover { color: #3730a3; border-color: #4338ca; }
.corner-btn.active { background: rgba(67,56,202,0.15); border-color: #4338ca; color: #3730a3; }
...
/* ── Axis box ───────────────────────────────────────────────────────────────── */
#vw-axisBox {
  position: absolute; top: 16px; left: 16px; width: 140px; height: 140px;
  pointer-events: none; z-index: 50; overflow: visible; display: none;
}
#vw-axisBox canvas { width: 100%; height: 100%; display: block; }

/* ── Selection info ─────────────────────────────────────────────────────────── */
#vw-selInfo {
  position: absolute; bottom: 70px; left: 50%; transform: translateX(-50%);
  background: rgba(255,255,255,0.88); backdrop-filter: blur(16px);
  border: 1px solid rgba(99,102,241,0.20); border-radius: 14px;
  padding: 6px 18px; font-size: 12px; color: #4338ca; font-weight: 600;
...
#vw-loading {
  ...
  background: rgba(241,245,249,0.85); backdrop-filter: blur(6px);
  flex-direction: column; align-items: center; justify-content: center; gap: 12px;
}
#vw-loading.show { display: flex; }
.spin { width:36px;height:36px;border-radius:50%;border:3px solid rgba(67,56,202,0.15);border-top-color:#4338ca;animation:vw-spin .8s linear infinite; }
@keyframes vw-spin { to{transform:rotate(360deg)} }
.load-txt { font-size:13px;color:#64748b; }
#vw-progTrack {
  width: 240px; height: 4px; background: rgba(99,102,241,0.12);
